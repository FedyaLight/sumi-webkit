...
@MainActor
final class ExtensionActionPopupFailureDiagnosticsTests: XCTestCase {
    func testClassifyReportsSourceResourcesMissingWhenResourceRootResolutionThrows() {
        ...
        let bucket = diagnostics.classifyActionPopupRuntimeFailure(
    ...
    func testDiagnosticLinesIncludeResourceRootResolutionError() {
        ...
        let lines = diagnostics.actionPopupRuntimeDiagnosticLines(
            extensionId: installedExtension.id,
            profileId: profileId,
            installedExtension: installedExtension,
