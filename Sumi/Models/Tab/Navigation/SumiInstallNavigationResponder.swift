...
final class SumiInstallNavigationResponder: SumiNavigationActionResponding, SumiNavigationResponseResponding {
    ...
    func decidePolicy(
        ...
        return tab?.navigationRuntime.installNavigationRuntime.interceptInstallNavigation(url) == true
            ? .cancel
            : .next
    }
    ...
    func decidePolicy(for navigationResponse: SumiNavigationResponse) async -> SumiNavigationResponsePolicy? {
        ...
        return tab?.navigationRuntime.installNavigationRuntime.interceptInstallNavigation(navigationResponse.url) == true
            ? .cancel
            : .next
    }
}
