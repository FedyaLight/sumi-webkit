    ...
    var encoded: Data? {
        ...
        do {
            ...
            RuntimeDiagnostics.debug(
                "SpaceGradient encoding failed: \(error)",
                category: "SpaceGradient"
            )
            return nil
        }
    }
...
extension SpaceGradient {
    func interpolated(to other: SpaceGradient, progress: Double) -> SpaceGradient {
        let clamped = min(max(progress, 0), 1)
        guard clamped > 0 else { return self }
        guard clamped < 1 else { return other }

        let from = SpaceGradient.encodeToAnimVector(self)
        let target = SpaceGradient.encodeToAnimVector(other)
        var blended = SpaceGradient.AnimVector()
        for index in 0..<SpaceGradient.AnimVector.width {
