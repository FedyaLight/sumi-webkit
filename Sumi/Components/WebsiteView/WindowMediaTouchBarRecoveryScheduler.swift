...
@MainActor
final class WindowMediaTouchBarRecoveryScheduler {
    ...
    func start() {
        ...
        recoveryCancellable = NotificationCenter.default
            ...
            .sink { [weak self] notification in
                self?.handleRecoveryRequest(notification)
            }
    }
    ...
    private func handleRecoveryRequest(_ notification: Notification) {
        guard let webView = notification.object as? WKWebView,
              let notificationWindowID = notification.userInfo?[
                  SumiMediaTouchBarRecoveryNotificationKey.windowID
              ] as? UUID,
        ...
        let tabID = notification.userInfo?[
            SumiMediaTouchBarRecoveryNotificationKey.tabID
        ] as? UUID
