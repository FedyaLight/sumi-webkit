...
struct CacheManagementPanel: View {
    ...
    let clearsLoadedDataOnDisappear: Bool
    ...
    var body: some View {
        ...
        .task(id: browserManager.currentProfile?.id) {
            await synchronizeManagerWithCurrentProfile()
        }
        .onDisappear {
            cancelPanelWork()
        }
    ...
    private func cancelPanelWork() {
        ...
        operationTask = nil
        if clearsLoadedDataOnDisappear {
    ...
    @MainActor
    private func synchronizeManagerWithCurrentProfile() async {
