...
struct MiniWindowWebView: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, WKScriptMessageHandler {
        ...
        func loadInitialURLIfNeeded(on webView: WKWebView) {
            ...
            RuntimeDiagnostics.emit("🔐 [MiniWindow] Loading URL: \(session.currentURL.absoluteString)")
        ...
        func checkForOAuthCompletion(url: URL) {
            ...
            if successIndicators.contains(where: { 
                ...
                RuntimeDiagnostics.emit("🔐 [MiniWindow] OAuth success detected: \(url.absoluteString)")
            ...
            if errorIndicators.contains(where: { 
                ...
                RuntimeDiagnostics.emit("🔐 [MiniWindow] OAuth error detected: \(url.absoluteString)")
            ...
            if let host = url.host?.lowercased(),
               ...
               !host.contains("discord.com") {
                ...
                RuntimeDiagnostics.emit("🔐 [MiniWindow] Possible OAuth redirect detected: \(url.absoluteString)")
                session.completeAuth(success: true, finalURL: url)
            }
        }
        ...
        func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
            ...
            RuntimeDiagnostics.emit("🔐 [MiniWindow] JavaScript auth completion detected: success=\(success), shouldClose=\(shouldClose), url=\(urlString ?? "nil")")
            ...
            if shouldClose {
                RuntimeDiagnostics.emit("🔐 [MiniWindow] Site requested window close, but keeping window open for user control")
            }
        }
    }
}
...
extension MiniWindowWebView.Coordinator: WKNavigationDelegate {
    func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
        RuntimeDiagnostics.emit("🔐 [MiniWindow] Navigation started: \(webView.url?.absoluteString ?? "nil")")
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        RuntimeDiagnostics.emit("🔐 [MiniWindow] Navigation finished: \(webView.url?.absoluteString ?? "nil")")
    ...
    func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        RuntimeDiagnostics.emit("🔐 [MiniWindow] Navigation failed: \(error.localizedDescription)")
        session.updateLoading(isLoading: false)
    }

    func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
        RuntimeDiagnostics.emit("🔐 [MiniWindow] Provisional navigation failed: \(error.localizedDescription)")
        session.updateLoading(isLoading: false)
    }
}
...
extension MiniWindowWebView.Coordinator: WKUIDelegate {
    ...
    func webView(
        ...
        RuntimeDiagnostics.emit("🎬 [MiniWindowWebView] Entering full-screen for video")
        
        // Get the window containing this webView
        guard let window = webView.window else {
            RuntimeDiagnostics.emit("❌ [MiniWindowWebView] No window found for full-screen")
    ...
    func webView(
        ...
        RuntimeDiagnostics.emit("🎬 [MiniWindowWebView] Exiting full-screen for video")
        
        // Get the window containing this webView
        guard let window = webView.window else {
            RuntimeDiagnostics.emit("❌ [MiniWindowWebView] No window found for exiting full-screen")
        ...
        DispatchQueue.main.async {
            if let window = webView.window {
                ...
                openPanel.beginSheetModal(for: window) { response in
                    RuntimeDiagnostics.emit("📁 [MiniWindowWebView] Open panel sheet completed with response: \(response)")
                    if response == .OK {
                        RuntimeDiagnostics.emit("📁 [MiniWindowWebView] User selected files: \(openPanel.urls.map { $0.lastPathComponent })")
                        completionHandler(openPanel.urls)
                    } else {
                        RuntimeDiagnostics.emit("📁 [MiniWindowWebView] User cancelled file selection")
                ...
                openPanel.begin { response in
                    RuntimeDiagnostics.emit("📁 [MiniWindowWebView] Open panel modal completed with response: \(response)")
                    if response == .OK {
                        RuntimeDiagnostics.emit("📁 [MiniWindowWebView] User selected files: \(openPanel.urls.map { $0.lastPathComponent })")
                        completionHandler(openPanel.urls)
                    } else {
