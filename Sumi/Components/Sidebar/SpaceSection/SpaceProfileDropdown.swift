        ...
        Menu {
            ForEach(profiles, id: \.id) { profile in
                Button(action: { onProfileSelected(profile.id) }) {
                    HStack(spacing: 8) {
                        SumiProfileGlyphDisplay(icon: profile.icon, font: .system(size: 13, weight: .semibold))
                        Text(profile.name)
                            .font(.system(size: 13))
                        Spacer()
            ...
            HStack(spacing: 6) {
                if let currentProfile = profiles.first(where: { $0.id == currentProfileId }) {
                    SumiProfileGlyphDisplay(icon: currentProfile.icon, font: .system(size: 13, weight: .semibold))
