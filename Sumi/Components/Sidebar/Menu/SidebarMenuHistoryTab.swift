...
struct SidebarMenuHistoryTab: View {
    ...
    var body: some View {
                ...
                HStack {
                    ...
                    TextField("Search history...", text: $text)
                        ...
                        .onChange(of: text) { _, newValue in
                            scheduleSearchHistory(for: newValue)
                        }
                ...
                Button {
                    ...
                    HStack(alignment: .center, spacing: 4) {
                        Image(systemName: "line.horizontal.3.decrease.circle")
                            ...
                            .foregroundStyle(
                                isShowingFilters
                                    ? tokens.buttonPrimaryText : contrastTextSecondary
                            )
                        Text("Filters")
                    ...
                    .background(
                        isShowingFilters
                            ? tokens.buttonPrimaryBackground : tokens.fieldBackground
                    )
                    .animation(
                    ...
                    if isLoading {
                        HStack {
                            ...
                            Text("Loading history...")
                                .font(.system(size: 12))
                                .foregroundColor(contrastTextSecondary)
                        }
        ...
        .onChange(of: selectedTimeRange) { _, _ in
            if text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                ...
                scheduleSearchHistory(for: text, delayNanoseconds: 120_000_000)
            }
        }
        .onDisappear {
            searchTask?.cancel()
    ...
    private func scheduleSearchHistory(
        for query: String,
...
struct HistoryRowView: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            if isHovered {
                HStack(spacing: 2) {
                    ...
                    .padding(8)
                    .background(
                        isTrashIconHovered ? tokens.fieldBackgroundHover : .clear
                    )
                    ...
                    .padding(8)
                    .background(
                        isArrowIconHovered ? tokens.fieldBackgroundHover : .clear
                    )
                    .clipShape(RoundedRectangle(cornerRadius: 8))
                    .buttonStyle(PlainButtonStyle())
        ...
        .background(
            RoundedRectangle(cornerRadius: 16)
                .fill(isHovered ? tokens.sidebarRowHover : .clear)
        )
        .clipShape(RoundedRectangle(cornerRadius: 12))
        ...
        .sumiAppKitContextMenu(entries: historyContextMenuEntries)
    }

    private func historyContextMenuEntries() -> [SidebarContextMenuEntry] {
        [
            .action(
    ...
    private func fetchFavicon() async {
        ...
        guard SumiFaviconResolver.cacheKey(for: entry.url) != nil else {
            await MainActor.run {
        ...
        guard let image = await SumiFaviconResolver.shared.image(for: entry.url) else {
            await MainActor.run {
                self.favicon = defaultFavicon
            }
            return
    ...
    var body: some View {
        Button {
            ...
            Text(text)
                .font(.system(size: 14, weight: .medium))
                .foregroundStyle(
                    isActive ? tokens.buttonPrimaryText : tokens.secondaryText
                )
                .lineLimit(1)
                .padding(.vertical, 10)
                .padding(.horizontal, 12)
                .background(
                    isActive
                        ...
                        : isHovering
                        ? tokens.fieldBackgroundHover : tokens.fieldBackground
                )
                .clipShape(RoundedRectangle(cornerRadius: 12))
        }
        .buttonStyle(.plain)
