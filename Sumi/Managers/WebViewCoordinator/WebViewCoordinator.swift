...
class WebViewCoordinator {
        ...
        for (tabId, webView) in webViewsToCleanup {
            ...
            if let tab = tabManager.tab(for: tabId) {
                ...
                performFallbackWebViewCleanup(
                    webView,
                    tabId: tabId,
                    extensionManager: tabManager.browserManager?.extensionManager
                )
            }
    ...
    private func performFallbackWebViewCleanup(
        ...
        if deferProtectedWebViewMutation(
            ...
            operation: { [weak self, weak webView, weak extensionManager] in
                guard let self, let webView else { return }
        ...
        extensionManager?.releaseExternallyConnectableRuntime(
            for: webView,
            reason: "fallback WebView cleanup"
        )
        ...
        let allMessageHandlers = Tab.coreScriptMessageHandlerNames(for: tabId)
