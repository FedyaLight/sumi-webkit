...
final class SumiPublicSuffixList: Sendable {
    /// The list parsed from the bundled `public_suffix_list.dat`, loaded once.
    static let bundled = SumiPublicSuffixList()
    ...
    func isPublicSuffix(_ host: String) -> Bool {
        let labels = host.components(separatedBy: ".")
    ...
    private static func bundledListText() -> String {
        let bundles = [Bundle(for: SumiPublicSuffixList.self), Bundle.main]
        for bundle in bundles {
            ...
            do {
                ...
                RuntimeDiagnostics.debug(
                    "Failed to read bundled Public Suffix List: \(error.localizedDescription)",
                    category: "PublicSuffixList"
                )
            }
        }
