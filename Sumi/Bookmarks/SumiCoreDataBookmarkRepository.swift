...
final class SumiCoreDataBookmarkRepository: SumiBookmarkRepository, @unchecked Sendable {
    ...
    /// drop the cache whenever the context's contents change (save, import,
    /// or a merge from another context). Lock-guarded because merges arrive
    /// on the saving context's queue.
    private nonisolated let snapshotCacheLock = NSLock()
    ...
    func snapshot(sortMode: SumiBookmarkSortMode = .manual) -> SumiBookmarksSnapshot {
        ...
        let cached = cachedSnapshots[sortMode]
        snapshotCacheLock.unlock()
        if let cached {
            return cached
        }

        let snapshot = buildSnapshot(sortMode: sortMode)
        snapshotCacheLock.lock()
        cachedSnapshots[sortMode] = snapshot
        snapshotCacheLock.unlock()
        return snapshot
    }

    private nonisolated func invalidateSnapshotCache() {
        snapshotCacheLock.lock()
        cachedSnapshots.removeAll(keepingCapacity: true)
        snapshotCacheLock.unlock()
    }

    private func buildSnapshot(sortMode: SumiBookmarkSortMode) -> SumiBookmarksSnapshot {
