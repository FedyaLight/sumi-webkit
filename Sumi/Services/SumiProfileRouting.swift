...
@MainActor
enum SumiProfileRouting {
    static func activeProfileId(
        for space: Space?,
    ...
    static func adoptProfileIfNeeded(
        ...
        guard let targetProfile = support.profileManager.profiles.first(where: { $0.id == targetProfileId })
            ...
            RuntimeDiagnostics.emit(
                "⚠️ [SumiProfileRouting] Window \(windowState.id) referenced missing profile \(targetProfileId); reset currentProfileId to \(fallbackId?.uuidString ?? "nil")."
            )
            return
        }
