...
final class WindowSessionService {
    ...
    func restoreWindowSession(
        ...
        guard let data = UserDefaults.standard.data(forKey: lastWindowSessionKey) else {
            return false
        }
        guard let snapshot = try? JSONDecoder().decode(WindowSessionSnapshot.self, from: data) else {
