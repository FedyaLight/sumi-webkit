...
schema_v2="$({
  sed -n \
    '/^enum SumiStartupSchemaV2: VersionedSchema {$/,/^enum SumiStartupMigrationPlan: SchemaMigrationPlan {$/p' \
    "$startup_persistence"
} || true)"
if [[ -z "$schema_v2" ]]; then
  guard_record_failure 'SumiStartupSchemaV2 declaration is missing'
else
  ...
  guard_exact 'startup schema V2 version declaration' "$schema_version_count" 1

  retirement_model_count="$({
    guard_count_matches 'ProfileRetirementEntity\.self' - <<< "$schema_v2"
  })"
  guard_exact 'retirement journal model in startup schema V2' "$retirement_model_count" 1
fi
