...
final class SumiLaunchSmokeUITests: XCTestCase {
    ...
    @MainActor
    private func launchApp(preferencesHomeURL: URL? = nil) -> XCUIApplication {
    ...
    private func activatePersonalSpace(
        ...
        let targetSpaceIcon: XCUIElement
        if spaceIcon.waitForExistence(timeout: 5) {
            targetSpaceIcon = spaceIcon
        } else {
            targetSpaceIcon = anySpaceIcon
        }
        targetSpaceIcon.coordinate(withNormalizedOffset: CGVector(dx: 0.5, dy: 0.5)).click()

        let title = element(withIdentifier: "space-title-\(fixture.personalSpaceID)", in: app)
        XCTAssertTrue(
            title.waitForExistence(timeout: 5),
            "Personal space title did not become available for \(fixture.personalSpaceID). Target icon was \(targetSpaceIcon.identifier)"
        )
    ...
    private func exerciseRegularTabDragAfterSourcePreservingContextMenuAction(
        ...
        performSidebarMarkerDrag(on: sourceBeforeAction, in: window)
        assertSidebarDragEventChain(
            sourceID: sourceElementID,
            expectedDragItemID: sourceTabID,
        ...
        try? FileManager.default.removeItem(at: markerURL)
        performSidebarMarkerDrag(on: targetAfterAction, in: window)
        assertSidebarDragEventChain(
    ...
    private func exerciseSidebarDragAfterSourcePreservingContextMenuAction(
        ...
        performSidebarMarkerDrag(on: controlAfterAction, in: window)
        assertSidebarDragEventChain(
        ...
        try? FileManager.default.removeItem(at: markerURL)
        performSidebarMarkerDrag(on: sourceAfterAction, in: window)
        assertSidebarDragEventChain(
            sourceID: sourceElementID,
            expectedDragItemID: expectedSourceDragItemID,
            expectedRouteOwnerView: postActionViewID,
    ...
    private func exerciseSidebarDragStartAfterContextMenuInteraction(
        ...
        try? FileManager.default.removeItem(at: sidebarDragMarkerFileURL())
        performSidebarMarkerDrag(on: target, in: window)
        ...
        performSidebarMarkerDrag(on: sourceAfterMenu, in: window)
        assertSidebarDragEventChain(
            sourceID: elementID,
    ...
    private func requireElement(
        ...
        XCTFail("Missing sidebar target \(identifier)", file: file, line: line)
        return target
    }
    ...
    private func revealHoverSidebar(in window: XCUIElement) {
        window.coordinate(withNormalizedOffset: CGVector(dx: 0.005, dy: 0.2)).hover()
    }
    ...
    private func assertSidebarDragEventChain(
        ...
        XCTAssertTrue(
            waitForSidebarMarkerEvent(named: "mouseDown", sourceID: sourceID, timeout: 3),
