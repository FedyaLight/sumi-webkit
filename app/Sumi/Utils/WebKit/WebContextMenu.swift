...
struct WebContextMenuCapture {
    let payload: WebContextMenuPayload
    let metadata: WebContextMenuMetadata

    init?(dictionary: [String: Any]) {
        guard let payload = WebContextMenuPayload(dictionary: dictionary) else {
            return nil
        }

        let pageURL = (dictionary["href"] as? String).flatMap(WebContextMenuPayload.makeURL)
        let frameURL = (dictionary["frameHref"] as? String).flatMap(WebContextMenuPayload.makeURL)
        let contextURL = (dictionary["contextHref"] as? String).flatMap(WebContextMenuPayload.makeURL)
            ?? frameURL
            ?? pageURL
        let inSubframe = (dictionary["inSubframe"] as? Bool) == true
        let fieldKind = dictionary["fieldKind"] as? String
        let capturedAt = Date(
            timeIntervalSince1970: ((dictionary["capturedAt"] as? Double) ?? (Date().timeIntervalSince1970 * 1000.0)) / 1000.0
        )

        self.payload = payload
        self.metadata = WebContextMenuMetadata(
            pageURL: pageURL,
            frameURL: frameURL,
...
enum WebContextMenuPayload {
    ...
    fileprivate static func makeURL(from string: String) -> URL? {
    ...
    fileprivate static func makeURLAllowingData(from string: String) -> URL? {
...
enum WebContextMenuItem {
    ...
    private static func content(for payload: WebContextMenuPayload) -> [WebContextMenuItem] {
        ...
        case .link:
            return [.linkOpenInNewTab, .linkOpenInGlance, .separator, .linkCopy, .separator, .systemShare, .separator, .systemInspect]
        case .image:
            return [
                .imageOpenInNewTab,
                .separator,
    ...
    private var title: String? {
        ...
        case .linkOpenInGlance: return "Open Link in Glance"
    ...
    private func isEnabled(context: FocusableWKWebView) -> Bool {
        ...
        case .linkOpenInGlance:
            return context.owningTab?.sumiSettings?.glanceEnabled ?? true
        default: return true
        }
    }
    ...
    private func performAction(on webView: FocusableWKWebView, payload: WebContextMenuPayload) {
        ...
        case .linkOpenInGlance:
            if let url = payload.linkURL, let tab = webView.owningTab {
...
extension FocusableWKWebView {
    func openLinkInNewTab(_ url: URL) {
        ...
        let context = BrowserManager.TabOpenContext.background(
            windowState: owningTab.flatMap { browserManager.windowState(containing: $0) },
            sourceTab: owningTab,
            preferredSpaceId: owningTab?.spaceId
        )
        _ = browserManager.openNewTab(url: url.absoluteString, context: context)
    }
