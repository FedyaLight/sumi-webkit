...
| 2026-06-30 | Deleted orphaned Boost zap preview chain (`previewZap`, `previewZapSelector`, `previewJavaScript`, and its JSON helper); active zap selection still uses `startZapSelection` and overlay JavaScript. | Periphery reported the chain as unused, and `rg` found only the chain's own declarations/calls. | Targeted Boost/sidebar/extension tests passed; `Sumi` Debug build passed. |
| 2026-06-30 | Removed unused private/UI helpers and fields: `NavButtonsView.windowState`, `PinnedTileVisual.reduceMotion`, `SidebarZenMotion.zenSidebarCompositeLifecycle`, `SidebarDragGeometryRepository.sidebarGeometryGeneration`, `PinnedTabsConfiguration.outlineMaskBleed`, `SumiProtectionCoordinator.cancellables`, the redundant `SumiContentBlockingService.compiler` field, `ExtensionManager.showErrorAlert`, `WebViewCoordinator.visibleTabIDs`, and the unread weak tab storage in `SafariExtensionInlineUINavigationResponder`. | Periphery reported each symbol as unused or assign-only, and `rg` found no live production or test references after preserving active runtime contracts. | Targeted sidebar/motion, Boost, extension placement, drag/drop tests passed; `Sumi` Debug build passed. |
| 2026-06-30 | Removed stale test-only contracts that kept dead production helpers alive: folder snapshot `bodyHeight`, pinned favicon corner-radius shim, sidebar row lifecycle animation accessor, drag operation wrapper, reverse drop-index projector, extension action visibility counter, and passive scroll indicator frame helpers. | Periphery reported the production helpers as unused; code review showed production computes these values directly or uses the remaining active helpers; stale assertions were updated or deleted. | Updated targeted tests passed; `rg` found no remaining references to the removed helper names. |
