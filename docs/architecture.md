...
receipt captured from its launcher container. Favorite execution keeps its
...
- Sidebar-first organization for tabs, spaces, pinned items, favorite, and
  folders.
...
Pinned and favorite items can also open through Glance-style launcher flows.

Favorite launchers are global across spaces that belong to the same profile. Pinned
items live in one space and look like normal tabs. Favorite appear as tiles.

## Launcher Semantics

Sumi separates visible organization from live page runtime where possible.
Pinned and favorite items can preserve their visible identity while the live
...
- Preserve visible tab, pinned, favorite, and folder organization when
