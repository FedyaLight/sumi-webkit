...
- Foreground main-frame external-scheme callbacks may prompt after redirect chains even when WebKit no longer reports a live user gesture; background frames and non-normal surfaces remain fail-closed, and automatic attempts are limited to one per page lifecycle.
...
| External schemes | `SumiExternalSchemePermissionBridgeTests`, `SumiPermissionPromptBridgeIntegrationTests`, `SumiNavigationResponderTests` | Foreground main-frame callbacks can prompt through the coordinator and launch only after allow; background frames, non-normal surfaces, repeated automatic attempts, unsupported schemes, and stored denies do not open. |
...
| External schemes | `mailto:`, `tel:`, `facetime:`, custom OAuth callback schemes, iframe/background attempts | Installed handler app, where applicable | URL-bar prompt for user-click and foreground main-frame redirect callbacks, URL hub External app links scheme rows, Privacy Site Settings | No app opens before allow; Open this time applies only to current attempt; iframe/non-normal-surface and repeated automatic attempts block; query/fragment redacted | Actual app availability depends on macOS handlers. |
