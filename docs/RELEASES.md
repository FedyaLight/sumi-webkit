...
- Tag: `v0.0.7`
- GitHub release title: `Sumi 0.0.7`
- GitHub release state: ordinary public Release, not draft and not GitHub prerelease
- Release DMG: `release/artifacts/0.0.7/Sumi-0.0.7-build12-macos-arm64.dmg`
...
```text
release/artifacts/0.0.7/Sumi-0.0.7-build12-macos-arm64.dmg
```
...
gh release create v0.0.7 \
  release/artifacts/0.0.7/Sumi-0.0.7-build12-macos-arm64.dmg \
  --title "Sumi 0.0.7" \
  --notes-file docs/releases/0.0.7.md \
  --latest
```
...
- Confirm version `0.0.7`, build `12`, channel `alpha`, feed URLs, and Apple development signing identity.
...
- Confirm both public feeds offer the same `0.0.7 build 12` Arm64 item after publication.
