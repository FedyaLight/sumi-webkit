...
struct SpacesSideBarView: View {
    ...
    private func recordUITestSidebarState(reason: String) {
        SidebarUITestDragMarker.recordEvent(
            ...
            details: "reason=\(reason) spaces=\(availableSpaces.count) currentSpace=\(windowState.currentSpaceId?.uuidString ?? "nil") sidebarVisible=\(windowState.isSidebarVisible) sidebarMenuVisible=\(windowState.isSidebarMenuVisible) presentationMode=\(String(describing: sidebarPresentationContext.mode))"
        )
    }
    ...
    private func spaceTransitionContainer(
        ...
        ZStack(alignment: .topLeading) {
            if transitionState.hasDestination,
            ...
            } else if let committedSpace = space(for: committedSpaceId(in: spaces), in: spaces) {
                makeSidebarPage(
                    for: committedSpace,
